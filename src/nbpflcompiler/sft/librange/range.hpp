...
{
  if (tree == NULL)
    tree = OpNode<KType,AType, SPType>::buildOpNode(default_action, op, key, action, sp); //QUI
  else
...
{
  typename std::map<AType,AType, SPType>::const_iterator i = mappings.find(default_action);
  if(i != mappings.end())
