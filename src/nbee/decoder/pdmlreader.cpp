...
struct _nbPDMLField *Result;
	...
	_nbPDMLProto *CurrentPDMLProto;
		...
		while (CurrentPDMLProto)
		{
			...
			{
				// The protoco we're looking for has been found; let's break this loop
	...
	if (FirstField)
	{
		...
		if (Result)
		{
			*ExtractedField= Result;
			...
			// There are no more occurrencies of the current field
			return nbWARNING;
	}
	else
	{
		// Let's scan the fields without allowing to go up in the hierarchy, because we're already
		// managing fields starting from the first available field within the selected protocol, so
		// there's not need to go up in the hierarchy
