...
*/
int NetPDLHexDumpToLong(unsigned char *HexDumpPtr, int ResultSize, int HexDumpIsInNetworkByteOrder)
{
...
/*!
	...
	\param HexDumpBin: a byffer that will contain the binary dump (according 
...
int DumpIndex;
	...
	// Check if the return buffer is big enough
	if (HexDumpBinSize >= (HexDumpAsciiSize*2))
		return nbFAILURE;
