...
//FULVIO TEMP 28/01/09 DA RIVEDERE
int nbDownloadAndUpdateNetPDLSchemaFile(const char* NetPDLSchemaFileLocation, char* ErrBuf, int ErrBufSize)
{
char* NetPDLSchemaDataBuffer;
FILE* NetPDLSchemaFileHandle;
size_t FileSize;
	...
	//FULVIO TEMP
	errorsnprintf(__FILE__, __FUNCTION__, __LINE__, ErrBuf, ErrBufSize, "Function nbDownloadAndUpdateNetPDLSchemaFile() not yet implemented");
	return nbFAILURE;

	if (nbDownloadNewNetPDLFile(&NetPDLSchemaDataBuffer, ErrBuf, ErrBufSize) == nbFAILURE)
		return nbFAILURE;

	// Get a temp file
	NetPDLSchemaFileHandle= tmpfile();
	if (NetPDLSchemaFileHandle == NULL)
	{
		nbGetLastErrorEx(__FILE__, __FUNCTION__, __LINE__,
			"Fatal error: cannot open a temp file to store the new NetPDL file.",
			ErrBuf, ErrBufSize);
	...
	FileSize= strlen(NetPDLSchemaDataBuffer);

	if (fwrite(NetPDLSchemaDataBuffer, sizeof(char), FileSize, NetPDLSchemaFileHandle) != FileSize)
	{
		nbGetLastErrorEx(__FILE__, __FUNCTION__, __LINE__,
			"Error writing data in the temp file.", ErrBuf, ErrBufSize);
	}

	fclose(NetPDLSchemaFileHandle);

	// If no errors occurred, it means that the content of the file is fine.
	// So, we can safely replace the old NetPDL file with the new one.

	if (NetPDLSchemaFileLocation)
	{
		NetPDLSchemaFileHandle= fopen(NetPDLSchemaFileLocation, "w");
//		TempNetPDLSchemaFileName= (char*) NetPDLSchemaFileLocation;
	}
	else
	{
	char NetPDLFilesDefaultLocation[1024];

		if (LocateLibraryLocation(NetPDLFilesDefaultLocation, sizeof(NetPDLFilesDefaultLocation), ErrBuf, ErrBufSize) == nbFAILURE)
			return nbFAILURE;

		NetPDLSchemaFileHandle= fopen(NetPDLFilesDefaultLocation, "w");
//		TempNetPDLSchemaFileName= NetPDLFilesDefaultLocation;
	}
