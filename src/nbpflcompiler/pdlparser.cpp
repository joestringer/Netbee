...
//#define PARSE_NOT_ONLY_SPECIFIED_PROTOCOL
#define VERIFY_ONLY_WITH_PAYLOAD
//#define ENCAP_ONLY_WITH_PAYLOAD
...
PDLParser::PDLParser(_nbNetPDLDatabase &protoDB, GlobalSymbols &globalSymbols, ErrorRecorder &errRecorder)
	...
	m_IsEncapsulation(0), m_ParentUnionField(0), m_BitFldCount(0), m_ScopeLevel(0),
	...
	case nbNETPDL_ID_EXPR_OPERAND_PROTOFIELD:
		{
...
#endif
				this->GenerateWarning(string("Operand '").append(operand->FieldName).append("' not defined"),
			...
			//if (m_CurrentParsingSection == PARSING_SECTION_EXEC)
			//{
			// fields used in the execution section will be used in any case
			//if (m_CurrProtoSym->ReferredFields[fieldSym] == NULL)
			//m_CurrProtoSym->ReferredFields[fieldSym] = 1;
			//}
	...
	case nbNETPDL_ID_EXPR_OPERAND_FUNCTION_EXTRACTSTRING:
		{
			...
			this->GenerateWarning(string("Function 'extractstring()' is not supported"), __FILE__, __FUNCTION__, __LINE__, 1, 5);
			return NULL;
		}break;
...
/*#else
			this->GenerateWarning(string("Regular Expression are currently disabled."),
				__FILE__, __FUNCTION__, __LINE__, 1, 5);
			return NULL;
#endif*/
	...
	case nbNETPDL_ID_FIELD_BIT:
		{
			...
			//StoreFieldSym(fieldBit->Name, fieldSymbol);	// [ds] store the field at the end
		}break;
...
void PDLParser::ParseSwitch(_nbNetPDLElementSwitch *switchElement)
{
	...
	{	
		...
		{
			this->GenerateWarning(string("String cases are not supported"), __FILE__, __FUNCTION__, __LINE__, 1, switchElement->NestingLevel);
			return;
		}
...
void PDLParser::ParseProtocol(SymbolProto &protoSymbol, HIRCodeGen &irCodeGen, bool visitEncap)
{
...

	{
		...
		bool isDefined = m_CurrProtoSym->IsDefined;
		m_CurrProtoSym->IsSupported = m_CurrProtoSym->IsDefined = true;

		ParseVerify(protoElement->FirstExecuteVerify);

		m_CurrProtoSym->VerifySectionSupported=(m_CurrProtoSym->IsSupported | m_CurrProtoSym->IsDefined);

#ifdef EXEC_BEFORE_AS_SOON_AS_FOUND
		if (protoElement->FirstExecuteBefore)
		{
			// just visit the before section to know if it's supported or not
			m_CurrProtoSym->IsSupported = m_CurrProtoSym->IsDefined = true;
...
#endif
		m_CurrProtoSym->IsSupported = isSupported;
		m_CurrProtoSym->IsDefined = isDefined;
	...
	*/
	...
	//m_CurrentParsingSection = PARSING_SECTION_EXEC;
...
//	}
	...
	//m_CurrentParsingSection = PARSING_SECTION_FORMAT;

	ParseElements(protoElement->FirstField);
...
void PDLParser::VisitElements(_nbNetPDLElementBase *firstElement)
{
	...
	this->GenerateInfo(string("Visiting elements in ").append(firstElement->ElementName).c_str(),
